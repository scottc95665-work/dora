...
node('dora-slave') {
    ...
    try {
        ...
        stage ('Build Tests Docker'){
           withDockerRegistry([credentialsId: '6ba8d05c-ca13-4818-8329-15d41a089ec0']) {
                buildInfo = rtGradle.run buildFile: './dora-api/docker-tests/build.gradle', tasks: 'dockerTestsPublish -DRelease=$RELEASE_PROJECT -DBuildNumber=$BUILD_NUMBER -DCustomVersion=$OVERRIDE_VERSION'
           }
        }
