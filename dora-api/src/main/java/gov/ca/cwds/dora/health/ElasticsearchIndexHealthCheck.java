...
public class ElasticsearchIndexHealthCheck extends ElasticsearchHealthCheck {
  ...
  static final String HEALTHY_ES_PLUGIN_MSG = "%s index exist on the server.";
  static final String UNHEALTHY_ES_PLUGIN_MSG = "%s index does not exist on the server.";
  ...
  protected Result check() throws Exception {
    ...
    try (RestClient esRestClient = DoraUtils.createElasticsearchClient(esConfig)) {
      List<Object> jsonMap = performRequestList(esRestClient, "GET", ES_INDEXES_ENDPOINT);

      if (!DoraUtils.isIndexExist(jsonMap, indexName)) {
        String unhealthyMsg = String
            .format(UNHEALTHY_ES_PLUGIN_MSG, indexName);
        LOGGER.error(unhealthyMsg);
        return Result.unhealthy(unhealthyMsg);
      } else {
        String healthyMsg = String.format(HEALTHY_ES_PLUGIN_MSG, indexName);
